...
public class FlechaControl extends AbstractControl {
    ...
    /**
 * Aplica efectos visuales iniciales según el tipo de flecha
 * ⭐ CORREGIDO: No sobrescribe texturas, solo ajusta brillo
 */
private void aplicarEfectoInicial() {
    // Guardar color base actual (puede ser White si hay textura, o un color sólido)
    ColorRGBA colorActual = material.getParamValue("Color");
    if (colorActual == null) {
        colorActual = ColorRGBA.White.clone();
    }
    
    switch (tipo) {
        case DORADA:
            // Brillo dorado (multiplica sobre la textura existente)
            material.setColor("Color", colorActual.mult(1.3f));
            break;
            
        case LUNA:
            // Brillo blanco-azulado
            material.setColor("Color", colorActual.mult(1.2f));
            break;
            
        case RAPIDA:
            // Brillo sutil rojizo
            material.setColor("Color", colorActual.mult(1.1f));
            break;
            
        default:
            // Normal: mantener color actual
            material.setColor("Color", colorActual);
            break;
    }
}
